...
class School_Portal():
    db_name = 'database management\students.db'

    def __init__(self,root):
        self.root = root
        self.root.title('Students Data')
        #----------logo and title-----------
        self.photo = PhotoImage(file = 'database management\icon.png')
        ...
        self.lblInfo = Label(font=('arial',10,'bold'),fg='black')
        ...
        chosser.add_cascade(label='File', menu=itemone)
        chosser.add_cascade(label='Add', command=self.add)
        chosser.add_cascade(label='Edit', command=self.edit)
        chosser.add_cascade(label='Delete',command = self.dele)
        chosser.add_cascade(label='Help',command = self.help)
        chosser.add_cascade(label='Exit',command = self.ex)

        root.config(menu=chosser)
    ...
    def add_record(self):
        if self.validation():
            ...
            self.message['text'] = 'Fields not completed! Fill all fields..XD'
    ...
    def edit_box(self):
        ...
        Label(self.edit_root,text = 'Age').grid(row = 11, column = 1, sticky = W)
    ...
    def edit_record(self,new_fname,fname,new_lname,lname,new_uname,uname,new_email,email,new_subject,subject,new_age,age):
        query = 'UPDATE StudentList SET First Name=?, Last Name=?, Username=?, Email=?, Subject=?, Age=? WHERE FirstName=? AND LastName=? AND Username=? AND Email=? AND Subject=? AND Age=?'
    ...
    def edit(self):
        ed = tkinter.messagebox.askquestion('Edit Records','Want to Edit a record?')
        if ed == 'yes':
            self.edit_box()
